...
crime_graph = np.zeros([len(day_list), 263, 20])
crime = pd.read_csv(input_path, low_memory=False)

top_20_class_name = crime['OFNS_DESC'].value_counts().index.to_list()[0:20]
...
for day in day_list:
    ...
    for index, row in one_day_data.iterrows():
        if row['OFNS_DESC'] in top_20_class_name:
            area_id = row['area_id']
            class_id = top_20_class_name.index(row['OFNS_DESC'])
            crime_graph[day_index][area_id][class_id] += 1
    print('{}: {} data finished'.format(time.time()-start_time,day))

np.save('/home/zh/crime_prediction_data/nyc/crime_counts/crime_counts_{}_byClass.npy'.format(data_year),crime_graph)